...
class Test_Policy():
    ...
    def full_friendship_testing(self, min_steps_since_last_test, attribute, value_list, agents, time_step):
        ...
        for agent in agents_copy:
            ...
            if(agent.neighbours):
                contact_list_copy = copy.copy(agent.neighbours)
    ...
    def run_tests(self,time_step):
        for machine in self.machine_list:
            if(not machine.is_empty() and not machine.is_running()):
                machine.run_tests(["Infected"],time_step)
    ...
    def release_results_to_agents(self,results):
        for result_obj in results:
            result_obj.agent.testing_hist.append(result_obj)
